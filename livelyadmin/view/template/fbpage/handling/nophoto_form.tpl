...
$('#btn-relax').bind('click', function(){
    if(confirm(<?php echo $text_confirm_relax ?>)){
        var res = resetTempLocker('index.php?route=fbpage/nophoto/ajax_data&set=1&token=<?php echo $token;?>',<?php echo $contribute_id;?>,<?php echo (int)$locker;?>);
        $('.do-result').html(res);
        location.reload();
    }
});
...
$('#btn-save').bind('click',function(){
    $.ajax({
        url:'index.php?route=fbpage/nophoto/save&token=<?php echo $token;?>',
        type:'POST',
...
<script type="text/javascript"><!-- 
$('#tab-history').load('index.php?route=fbpage/nophoto/history&token=<?php echo $token;?>&contribute_id=<?php echo $contribute_id;?>');
