	                                ...
	                                <?php if($modify) { ?>
	                                    <div class="input-group">
	                                        ...
	                                        <span class="modified input-group-addon btn">
	                                            <i class="fa fa-pencil"></i>
	                                            <input type="checkbox" name="product_modified" value="1"/>
	                                        </span>
	                                    </div>
...
<?php if($relax){?>
 $('#btn-relax').bind('click', function(){
    if(confirm('<?php echo $text_confirm_relax ?>')){
        var res = resetTempLocker('index.php?route=fbaccount/publish_nophoto/ajax_data&set=1&token=<?php echo $token;?>',<?php echo $contribute_id;?>,<?php echo (int)$locker;?>);
        $('.do-result').html(res);
        location.reload();
    }
});
...
$('#tab-history').load('index.php?route=fbaccount/publish_nophoto/history&token=<?php echo $token;?>&contribute_id=<?php echo $contribute_id;?>');
...
$('#btn-save').bind('click',function(){
    ...
    $.ajax({
        url:'index.php?route=fbaccount/publish_nophoto/save&token=<?php echo $token;?>',
        type:'POST',
...
$('.level-copy').bind('change',function(){
    $('.btn-copy').toggleClass('hidden',($.inArray(parseInt($(this).val()),level_status)==-1));
});
$('.btn-copy').bind('click',function(){
    ...
    if(confirm('Copy to with photo?')){
        $.ajax({
            ...
            dataType:'json',
            beforeSend:function(){
                $('.do-result').html('<div class="alert alert-warning"><img src="<?php echo TPL_IMG?>loading.gif" class="loading"></div>');
            },
            success:function(data){
                $('.do-result').empty();
                if(data.status == 0){
                    ...
                    $('.do-result').html('<div class="alert alert-success">'+data.msg+' <b>New SN: '+data.last+'</b></div>');
                }
            }
        });                   
    }   
