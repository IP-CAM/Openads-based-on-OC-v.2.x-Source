<?php
class ControllerFbmessageProfileNophoto extends Controller {
	private $error = array();

	public function index() {
		$this->language->load('fbmessage/profile_nophoto');
			
		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('fbmessage/profile_nophoto');
		...
		$data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('fbmessage/profile_nophoto', 'token=' . $this->session->data['token'] , 'SSL'),
        );
        $data['delete'] = $this->url->link('fbmessage/profile_nophoto/delete', 'token=' . $this->session->data['token'].$url , 'SSL');
		$limit = 20;
        ...
        $total = $this->model_fbmessage_profile_nophoto->getTotalContributes($filter_data);

        $results = $this->model_fbmessage_profile_nophoto->getContributes($filter_data);
        ...
        foreach ($results as $result) {
            ...
            if($lock){
                $action[] = array(
                    ...
                    'href' => $this->url->link('fbmessage/profile_nophoto/detail', 'token=' . $this->session->data['token'] .'&contribute_id='.$result['contribute_id'], 'SSL')
                );
            }else{
            	$action[] = array(
        ...
        $data['sort_product'] = $this->url->link('fbmessage/profile_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.product_id' . $url, 'SSL');
        $data['sort_contribute_sn'] = $this->url->link('fbmessage/profile_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.contribute_sn' . $url, 'SSL');
        $data['sort_submited_date'] = $this->url->link('fbmessage/profile_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.submited_date' . $url, 'SSL');
        $data['sort_date_modified'] = $this->url->link('fbmessage/profile_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.date_modified' . $url, 'SSL');
        $data['sort_user'] = $this->url->link('fbmessage/profile_nophoto', 'token=' . $this->session->data['token'] . '&sort=user' . $url, 'SSL');
        $data['sort_status'] = $this->url->link('fbmessage/profile_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, 'SSL');
        $data['sort_entry_sn'] = $this->url->link('fbmessage/profile_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.entry_sn' . $url, 'SSL');
        $url = '';
        if (isset($this->request->get['filter_entry'])) {
        	$url .= '&filter_entry=' . urlencode(html_entity_decode($this->request->get['filter_entry'], ENT_QUOTES, 'UTF-8'));
        }
        ...
        $pagination->url = $this->url->link('fbmessage/profile_nophoto', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
        ...
        $this->response->setOutput($this->load->view('fbmessage/profile/nophoto_list.tpl', $data));
	}
	...
	public function detail(){
        ...
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->url->link('fbmessage/profile_nophoto', 'token=' . $this->session->data['token'] . $url, 'SSL')
        );
        $data['cancel'] = $this->url->link('fbmessage/profile_nophoto', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $data['action'] = $this->url->link('fbmessage/profile_nophoto/save', '&token=' . $this->session->data['token'] , 'SSL');
        ...
        $_post = $this->model_fbmessage_profile_nophoto->getContribute($this->request->get['contribute_id']);
        ...
        //modify lock
        $data['locked'] = false;  
        $data['edit'] = ($this->user->getId() == $author_user) && !in_array($_post['status'], $this->config->get('fbmessage_level_status'));
        ...
        if(empty($data['lock_user']) || $_post['locker'] == $this->user->getId()){
            $this->model_fbmessage_profile_nophoto->setTempLocker((int)$_post['contribute_id']); 
        }else{
            $data['locked'] = true;
            $data['edit'] = false;
        ...
        $this->response->setOutput($this->load->view('fbmessage/profile/nophoto_form.tpl', $data));
	}
	...
	public function save(){
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			$_post = $this->model_fbmessage_profile_nophoto->getContribute($this->request->post['contribute_id']);
			...
			if($this->model_fbmessage_profile_nophoto->editContribute($tmp)){
	...
	public function history(){
		...
		$total = $this->model_fbmessage_profile_nophoto->getTotalHistory($contribute_id);
		$results = $this->model_fbmessage_profile_nophoto->getHistories($contribute_id,($page - 1) * $limit, $limit);
		$this->load->model('fbmessage/nophoto_status');
		foreach ($results as $result) {
		...
		$pagination->page = $page;
		$pagination->limit = $limit;
		$pagination->url = $this->url->link('fbmessage/profile_nophoto/history', 'token=' . $this->session->data['token'] . '&contribute_id='.$contribute_id . '&page={page}', 'SSL');
		$data['pagination'] = $pagination->render();
		$data['results'] = sprintf($this->language->get('text_pagination'), ($total) ? (($page - 1) * $limit) + 1 : 0, ((($page - 1) * $limit) > ($total - $limit)) ? $total : ((($page - 1) * $limit) + $limit), $total, ceil($total / $limit));
		
		$this->response->setOutput($this->load->view('fbmessage/profile/nophoto_history.tpl', $data));
	}
    ...
    function ajax_data(){
        ...
        switch ($action) {
            case 'reset':
                $this->model_fbmessage_profile_nophoto->resetTempLocker($this->request->post['contribute_id'],$this->request->post['locker']);
                if(isset($this->request->get['set']) && $this->request->get['set']){
                    $this->model_fbmessage_profile_nophoto->setTempLocker($this->request->post['contribute_id']);
                }
            ...
            case 'get':
                ...
                if($status){
                    $total = $this->model_fbmessage_profile_nophoto->getTotalContributes(array('filter_uncopied_status'=>$status));
