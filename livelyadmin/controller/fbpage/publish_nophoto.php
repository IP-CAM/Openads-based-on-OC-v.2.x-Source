<?php
class ControllerFbpagePublishNophoto extends Controller {
    private $error = array();

    public function index() {
        $this->language->load('fbpage/publish_nophoto');
            
        $this->document->setTitle($this->language->get('heading_title'));

        $this->load->model('fbpage/publish_nophoto');
                ...
                if(is_array($selecteds)){
                    foreach ($selecteds as $contribute_id) {
                        if((int)$contribute_id){
                            $_post = $this->model_fbpage_publish_nophoto->getContribute($contribute_id);
                            $tmp = array();
                            ...
                            if($tmp){
                                ...
                                if(!$this->model_fbpage_publish_nophoto->editContribute($tmp,'bulk')){
                                    $failed[$contribute_id] = $contribute_id;
                                    continue;
                                }
                            }else{
                                $failed[$contribute_id] = $contribute_id;
    ...
    public function delete() {
        ...
        if (isset($this->request->post['selected']) && $this->validateDelete()) {
            foreach ($this->request->post['selected'] as $post_id) {
                $this->model_fbpage_publish_nophoto->deleteContribute($post_id);
            }
            ...
            $this->response->redirect($this->url->link('fbpage/publish_nophoto', 'token=' . $this->session->data['token'] . $url, 'SSL'));
        }
    ...
    protected function validateDelete() {
        if (!$this->user->hasPermission('modify', 'fbpage/publish_nophoto/delete')) {
    ...
    protected function getList() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('fbpage/publish_nophoto', 'token=' . $this->session->data['token'] , 'SSL'),
        );

        $data['delete'] = $this->url->link('fbpage/publish_nophoto/delete', 'token=' . $this->session->data['token'].$url , 'SSL');
        $limit = 20;
        ...
        $total = $this->model_fbpage_publish_nophoto->getTotalContributes($filter_data);

        $results = $this->model_fbpage_publish_nophoto->getContributes($filter_data);
        ...
        foreach ($results as $result) {
            ...
            if($lock){
                $action[] = array(
                    ...
                    'href' => $this->url->link('fbpage/publish_nophoto/detail', 'token=' . $this->session->data['token'] .'&contribute_id='.$result['contribute_id'], 'SSL')
                );
            }else{
                $action[] = array(
            ...
            $data['contributes'][] = array(
                ...
                'entry_name'    => empty($entry['entry_name']) ? '' : $result['entry_sn'].' '.$entry['entry_name'] ,
        ...
        $data['sort_entry_sn'] = $this->url->link('fbpage/publish_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.entry_sn' . $url, 'SSL');
        $data['sort_submited_date'] = $this->url->link('fbpage/publish_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.submited_date' . $url, 'SSL');
        $data['sort_user'] = $this->url->link('fbpage/publish_nophoto', 'token=' . $this->session->data['token'] . '&sort=user' . $url, 'SSL');
        $data['sort_status'] = $this->url->link('fbpage/publish_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, 'SSL');
        $data['sort_publish'] = $this->url->link('fbpage/publish_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.publish' . $url, 'SSL');
        $data['sort_product'] = $this->url->link('fbpage/publish_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.product_id' . $url, 'SSL');
        $data['sort_author'] = $this->url->link('fbpage/publish_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.author_id' . $url, 'SSL');
        $url = '';
        if (isset($this->request->get['filter_entry'])) {
            $url .= '&filter_entry=' . urlencode(html_entity_decode($this->request->get['filter_entry'], ENT_QUOTES, 'UTF-8'));
        }
        ...
        $pagination->url = $this->url->link('fbpage/publish_nophoto', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
        ...
        $this->response->setOutput($this->load->view('fbpage/publish/nophoto_list.tpl', $data));
    }
    ...
    public function detail(){
        ...
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->url->link('fbpage/publish_nophoto', 'token=' . $this->session->data['token'] . $url, 'SSL')
        );
        $data['cancel'] = $this->url->link('fbpage/publish_nophoto', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $data['action'] = $this->url->link('fbpage/publish_nophoto/save', '&token=' . $this->session->data['token'] , 'SSL');
        ...
        $_post = $this->model_fbpage_publish_nophoto->getContribute($this->request->get['contribute_id']);
        ...
        $data['modify'] = in_array($this->user->getId(), $superior) && !in_array($_post['publish'], array($this->config->get('fbpage_promotion_modify')));
        ...
        if(empty($data['lock_user']) || ((int)$_post['locker'] == $this->user->getId()) ){
            $this->model_fbpage_publish_nophoto->setTempLocker((int)$_post['contribute_id']);
        }else {
            $data['locked'] = true;
            $data['modify'] = false;
        ...
        $data['promotion_modifies'] = $this->config->get("fbpage_promotion_modify");

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');

        $this->response->setOutput($this->load->view('fbpage/publish/nophoto_form.tpl', $data));
    }
    public function save(){
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            $_post = $this->model_fbpage_publish_nophoto->getContribute($this->request->post['contribute_id']);
            ...
            $this->model_fbpage_publish_nophoto->editContribute($tmp,'modified');
    ...
    public function history(){
        ...
        $total = $this->model_fbpage_publish_nophoto->getTotalHistory($contribute_id);
        $results = $this->model_fbpage_publish_nophoto->getHistories($contribute_id,($page - 1) * $limit, $limit);
        foreach ($results as $result) {
        ...
        $pagination->page = $page;
        $pagination->limit = $limit; 
        $pagination->url = $this->url->link('fbpage/publish_nophoto/history', 'token=' . $this->session->data['token'] . '&contribute_id='.$contribute_id . '&page={page}', 'SSL');
            
        $data['pagination'] = $pagination->render();
        
        $data['results'] = sprintf($this->language->get('text_pagination'), ($total) ? (($page - 1) * $limit) + 1 : 0, ((($page - 1) * $limit) > ($total - $limit)) ? $total : ((($page - 1) * $limit) + $limit), $total, ceil($total / $limit));
        
        $this->response->setOutput($this->load->view('fbpage/publish/nophoto_history.tpl', $data));
    }
    ...
    function ajax_data(){
        ...
        switch ($action) {
            case 'reset':
                $this->model_fbpage_publish_nophoto->resetTempLocker($this->request->post['contribute_id'],$this->request->post['locker']);
                if(isset($this->request->get['set']) && $this->request->get['set']){
                    $this->model_fbpage_publish_nophoto->setTempLocker($this->request->post['contribute_id']);
                }
