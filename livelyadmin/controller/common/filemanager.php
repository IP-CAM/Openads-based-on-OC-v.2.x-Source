...
class ControllerCommonFileManager extends Controller {
	...
	public function upload() {
		...
		if (!$json) {
			if (!empty($this->request->files['file']['name']) && is_file($this->request->files['file']['tmp_name'])) {
			...
			} else if(isset($this->request->files['attachment']) && $this->request->files['attachment']['tmp_name']){
				$filename = basename(html_entity_decode($this->request->files['attachment']['name'], ENT_QUOTES, 'UTF-8'));
				
				if ((strlen($filename) < 3) || (strlen($filename) > 255)) {
					$json['error'] = $this->language->get('error_filename');
				}
				$tempFile = $this->request->files['attachment']['tmp_name'];
				$basePath = DIR_UPLOAD;
				...
				$fileInfo = pathinfo($this->request->files['attachment']['name']);
				
				$fileName = date('His')."_".substr(md5(uniqid()),rand(0, 15),8).'.'.$fileInfo['extension'];
				$targetFile = rtrim($targetPath,'/') . '/'. $fileName;
				 
				if(!isset($this->request->get['type'])){
					$allowed = array('jpg','jpeg','gif','png','doc','docx','txt','facebook','zip','csv');
				}else{
				...
				if (in_array($fileInfo['extension'],$allowed)) {
					if(@move_uploaded_file($tempFile,$targetFile)){
						...
						$json['path'] = '../asset/upload/'.date('Ymd',time()).'/'.$fileName;
	...
	public function delete() {
		...
		if (!$this->user->hasPermission('common/filemanager/delete')) {
