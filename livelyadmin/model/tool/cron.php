...
///---------------------------------------------------------------------------------------前台判断广告词的内容与标题相似度
				...
				if($Aresult){
					...
					if((int)$resulpercent <= (int)$configPercent && (int)$resultTitlePercent<=(int)$titleConfigPercent){
						...
						$this->db->query("INSERT INTO ".DB_PREFIX.$log_history." SET `advertise_id` = '".$newContent['advertise_id']."',`post_id` = '".$newContent['post_id']."',`from` = 'backend',`status` = '4', `user_id` = '0',date_added = NOW() ");
							$df="UPDATE ".DB_PREFIX.$base_table." SET `status` = '4',`note`='',date_modified = NOW() WHERE `post_id` = '".$newContent['post_id']."'";
							$ff="INSERT INTO ".DB_PREFIX.$log_history." SET `advertise_id` = '".$newContent['advertise_id']."',`post_id` = '".$newContent['post_id']."',`from` = 'backend',`status` = '4', `user_id` = '0',date_added = NOW() ";
	...
	public function cron_log($msg,$contribite_id){
		$this->db->query("INSERT INTO ".DB_PREFIX."cron_log SET `contribute_id`='".$contribite_id."', `action`='".$this->db->escape($msg)."' , added_date=NOW()");
	}
}