<?php
class ModelFbpageNophoto extends Model {
    ...
    public function getContributes($data) {
        $sql = "SELECT p.* FROM " . DB_PREFIX . "fbpage_nophoto_post p WHERE 1 ";
        $implode = array();
        ...
        if(isset($data['filter_url_operator']) && $data['filter_url_operator']!== false && !empty($data['filter_url_number'])){
            $implode[] = " p.contribute_sn IN ( SELECT pu.contribute_sn FROM ".DB_PREFIX."fbpage_nophoto_post_url pu LEFT JOIN ".DB_PREFIX."fbpage_nophoto_post np ON np.contribute_sn = pu.contribute_sn WHERE np.publish ='".$this->post_type."' GROUP BY pu.contribute_sn HAVING COUNT(pu.contribute_sn) ".$data['filter_url_operator']." '".(int)$data['filter_url_number']."' )";
        }
    ...
    public function getTotalContributes($data) {
        $sql = "SELECT COUNT(p.contribute_id) AS total  FROM " . DB_PREFIX . "fbpage_nophoto_post p WHERE 1";
        $implode = array();
        // 根据当前登录用户,获取用户的下属用户的author_id
        if(!in_array($this->user->getId(), $this->config->get("sns_group_admin"))){
            $authors = array("'".$this->user->getAuthorId()."'");
    ...
    public function getContribute($contribute_id) {
        $sql = "SELECT * FROM " . DB_PREFIX . "fbpage_nophoto_post WHERE contribute_id = '" . (int)$contribute_id . "'";
        $query = $this->db->query($sql);
        return $query->row;
    }
    ...
    public function approve($data){  
        ...
        if(isset($data['status'])){
            ...
            }else if(!in_array($data['status'], $this->config->get("fbpage_level_status")) 
                && $info['publish'] != $this->config->get("fbpage_initial_publish") ){
                $fields['publish'] = $this->config->get("fbpage_initial_publish");
                $log['post'] = $this->config->get("fbpage_initial_publish");
            }
        }
