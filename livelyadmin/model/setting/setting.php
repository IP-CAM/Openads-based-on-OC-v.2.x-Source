...
class ModelSettingSetting extends Model {
	...
	public function editItemValue($key,$value,$code = 'config'){
		$serialized = 0;
		if(is_array($value)){
		...
		if($query->num_rows){
			$result = $this->db->update("setting",array('key'=>$key),array('value'=>$value));
		}else{
			$result = $this->db->insert("setting",array('code' => $code, 'key' => $key, 'value' => $value,'serialized' => $serialized));
		}
		return $result ;
	}
}
