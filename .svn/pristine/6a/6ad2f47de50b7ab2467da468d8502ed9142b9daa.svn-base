<?php
class ControllerFbaccountPublishNophoto extends Controller {
	private $error = array();

	public function index() {
		$this->language->load('fbaccount/publish_nophoto');
			
		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('fbaccount/publish_nophoto');
				...
				if(is_array($selecteds)){
					foreach ($selecteds as $contribute_id) {
						if((int)$contribute_id){
							$_post = $this->model_fbaccount_publish_nophoto->getContribute($contribute_id);
							$tmp = array();
							...
							if($tmp){
								...
								if(!$this->model_fbaccount_publish_nophoto->editContribute($tmp,'bulk')){
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $post_id) {
				$this->model_fbaccount_publish_nophoto->deleteContribute($post_id);
			}
			...
			$this->response->redirect($this->url->link('fbaccount/publish_nophoto', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function validateDelete() {
		if (!$this->user->hasPermission('modify', 'fbaccount/publish_nophoto/delete')) {
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('fbaccount/publish_nophoto', 'token=' . $this->session->data['token'] , 'SSL'),
        );
        $data['delete'] = $this->url->link('fbaccount/publish_nophoto/delete', 'token=' . $this->session->data['token'].$url , 'SSL');
		$limit = 20;
        ...
        $total = $this->model_fbaccount_publish_nophoto->getTotalContributes($filter_data);

        $results = $this->model_fbaccount_publish_nophoto->getContributes($filter_data);
        ...
        foreach ($results as $result) {
            ...
            if($lock){
                $action[] = array(
                    ...
                    'href' => $this->url->link('fbaccount/publish_nophoto/detail', 'token=' . $this->session->data['token'] .'&contribute_id='.$result['contribute_id'], 'SSL')//.'&mode=approve'
                );
            }else{
            	$action[] = array(
                	'text' => $this->language->get('text_edit'),
                	'href' => $this->url->link('fbaccount/publish_nophoto/detail', 'token=' . $this->session->data['token'] .'&contribute_id='.$result['contribute_id'], 'SSL')
                );
            }
        ...
        $data['sort_product'] = $this->url->link('fbaccount/publish_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.product_id' . $url, 'SSL');
        $data['sort_contribute_sn'] = $this->url->link('fbaccount/publish_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.contribute_sn' . $url, 'SSL');
        $data['sort_submited_date'] = $this->url->link('fbaccount/publish_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.submited_date' . $url, 'SSL');
        $data['sort_date_modified'] = $this->url->link('fbaccount/publish_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.date_modified' . $url, 'SSL');
        $data['sort_user'] = $this->url->link('fbaccount/publish_nophoto', 'token=' . $this->session->data['token'] . '&sort=user' . $url, 'SSL');
        $data['sort_status'] = $this->url->link('fbaccount/publish_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, 'SSL');
        $data['sort_publish'] = $this->url->link('fbaccount/publish_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.publish' . $url, 'SSL');
        $data['sort_entry_sn'] = $this->url->link('fbaccount/publish_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.entry_sn' . $url, 'SSL');
        $data['sort_author'] = $this->url->link('fbaccount/publish_nophoto', 'token=' . $this->session->data['token'] . '&sort=p.author_id' . $url, 'SSL');
        $url = '';
        if (isset($this->request->get['filter_entry'])) {
        	$url .= '&filter_entry=' . urlencode(html_entity_decode($this->request->get['filter_entry'], ENT_QUOTES, 'UTF-8'));
        }
        ...
        $pagination->url = $this->url->link('fbaccount/publish_nophoto', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
        ...
        $this->response->setOutput($this->load->view('fbaccount/publish/nophoto_list.tpl', $data));
	}
	...
	public function detail(){
        ...
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->url->link('fbaccount/publish_nophoto', 'token=' . $this->session->data['token'] . $url, 'SSL')
        );
        $data['cancel'] = $this->url->link('fbaccount/publish_nophoto', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $data['action'] = $this->url->link('fbaccount/publish_nophoto/save', '&token=' . $this->session->data['token'] , 'SSL');
        ...
        $_post = $this->model_fbaccount_publish_nophoto->getContribute($this->request->get['contribute_id']);
        ...
        $data['links'] = $this->model_fbaccount_publish_nophoto->getContributeLinks($_post['contribute_sn']);
        ...
        if(empty($data['lock_user']) || $_post['locker'] == $this->user->getId()){
            $this->model_fbaccount_publish_nophoto->setTempLocker((int)$_post['contribute_id']); 
        }else{
            $data['locked'] = true;
            $data['modify'] = false;
        ...
        $this->response->setOutput($this->load->view('fbaccount/publish/nophoto_form.tpl', $data));
	}
	...
	public function save(){
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			$_post = $this->model_fbaccount_publish_nophoto->getContribute($this->request->post['contribute_id']);
			...
			if($this->model_fbaccount_publish_nophoto->editContribute($tmp,'modified')){
	...
	public function history(){
		...
		$pagination->page = $page;
		$pagination->limit = $limit;
		$pagination->url = $this->url->link('fbaccount/publish_nophoto/history', 'token=' . $this->session->data['token'] . '&contribute_id='.$contribute_id . '&page={page}', 'SSL');
		$data['pagination'] = $pagination->render();
		$data['results'] = sprintf($this->language->get('text_pagination'), ($total) ? (($page - 1) * $limit) + 1 : 0, ((($page - 1) * $limit) > ($total - $limit)) ? $total : ((($page - 1) * $limit) + $limit), $total, ceil($total / $limit));
		
		$this->response->setOutput($this->load->view('fbaccount/publish/nophoto_history.tpl', $data));
	}
		...
		switch ($mode) {
			case 'normal':
				...
				while ($data = fgetcsv($fp)) {
					...
					$_post = $this->model_fbaccount_publish_nophoto->getContributeBySn(trim($data[0]));
					if(isset($_post['contribute_id'])){
						if(is_numeric($data[1])){
							$status = (int)$data[1];
						}else{
							$status_info = $this->model_fbaccount_nophoto_status->getStatusIdByName(trim($data[1]));
							$status = empty($status_info['status_id']) ? 0 : (int)$status_info['status_id'];
						}
						$this->model_fbaccount_publish_nophoto->editContribute(array('contribute_id'=>(int)$_post['contribute_id'],'publish'=>$status),'modified');
						$n++;
					}
				}
				fclose($fp);
			...
			case 'dele_links':
				...
				if(count($urls_list)){
					$num=count($urls_list);
					for($i=0;$i<$num;$i++){
						$res=$this->model_fbaccount_publish_nophoto->deleUrl($urls_list[$i]['0']);
						if($res){
							$deleNum++;
						}
					}
			...
			case 'links':
				...
				if(count($url_key)){
					foreach ($url_key as $key=>$value){
						$this->model_fbaccount_publish_nophoto->emptyUrlByPost($key);
					}
					$num=count($urls_list);
					for($i=0;$i<$num;$i++){
						$requ=$this->model_fbaccount_publish_nophoto->getContributeBySn($urls_list[$i]['0']);
						if($requ){
							if($requ['url_modified']==0){
								$res=$this->model_fbaccount_publish_nophoto->importUrl($urls_list[$i]['0'], $urls_list[$i]['1']);
								if($res){
									$importNum++;
								}
							}
    ...
    function ajax_data(){
        ...
        switch ($action) {
            case 'reset':
                $this->model_fbaccount_publish_nophoto->resetTempLocker($this->request->post['contribute_id'],$this->request->post['locker']);
                if(isset($this->request->get['set']) && $this->request->get['set']){
                    $this->model_fbaccount_publish_nophoto->setTempLocker($this->request->post['contribute_id']);
                }
            ...
            case 'get':
                ...
                if($status){
                    $total = $this->model_fbaccount_publish_nophoto->getTotalContributes(array('filter_uncopied_status'=>$status));
            ...
            case 'copy':
                ...
                $results = array();
                if($status){
                    $results = $this->model_fbaccount_publish_nophoto->getContributes(array('filter_uncopied_status'=>$status));
                }else if($contribute_id){
                    $contribute = $this->model_fbaccount_publish_nophoto->getContribute($contribute_id);   
                ...
                if($results){
                    foreach ($results as $item) {
                        $sn = $this->model_fbaccount_publish_nophoto->copyContribute($item,true);
                        if($sn){
                            $last = $sn;
                            $n++;
                        }else{
