<?php
class ModelFbaccountNophoto extends Model {
	private $post_type = 2;
	...
	public function deleteContribute($contribute_id){
		...
		$this->db->delete("fbaccount_nophoto_text_history",array('post_id' => (int)$contribute_id));
		return true;
	}

	public function getContributes($data) {

		$sql = "SELECT p.* FROM " . DB_PREFIX . "fbaccount_nophoto_post p WHERE 1 ";
		$implode = array();
	...
	public function getTotalContributes($data) {
		$sql = "SELECT COUNT(p.contribute_id) AS total  FROM " . DB_PREFIX . "fbaccount_nophoto_post p WHERE 1";
		$implode = array();
        // 根据当前登录用户,获取用户的下属用户的author_id
        if(!in_array($this->user->getId(), $this->config->get("sns_group_admin"))){
            $authors = array("'".$this->user->getAuthorId()."'");
