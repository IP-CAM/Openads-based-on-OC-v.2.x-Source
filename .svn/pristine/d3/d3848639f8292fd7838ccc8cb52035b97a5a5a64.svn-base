            ...
            <div class="panel-body">
                ...
                <div class="well" id="filter-column" <?php echo $filter_column ? '' : 'style="display:none ;"'?>>
                    <div class="row filter">
                        ...
                        <div class="col-sm-2">
                            <div class="form-group">
                                <label class="control-label"><?php echo $column_author ?></label>
                        ...
                        <div class="col-sm-2">
                            ...
                            <div class="form-group">
                                <label class="control-label"><?php echo $column_publish ?></label>
...
<!--	
function filter() {
	url = 'index.php?route=fbaccount/publish_nophoto&token=<?php echo $token; ?>';
	var paramArr=[];
        ...
        buttons:{
            'Import':function(){
                $('#import-dialog div.tab-on form').ajaxSubmit({
                    url:'index.php?route=fbaccount/publish_nophoto/import_data&token=<?php echo $token;?>',
                    type:'Post',
                    dataType:'json',
                    beforeSubmit:function(){
                        $('#import-dialog div.tab-on .do-result').html('<div class="alert alert-warning"><img src="<?php echo TPL_IMG ?>loading.gif"> Please Waiting ... </div>');
                    },
...
$('#btn-export').bind('click',function(){
    $.ajax({
        url:'index.php?route=fbaccount/publish_nophoto/advanced_export&token=<?php echo $token;?>',
        type:'POST',
        ...
        buttons:{
            'Save':function(){
                $('#bulk-form').ajaxSubmit({
                    url:'index.php?route=fbaccount/publish_nophoto/bulk&token=<?php echo $token;?>',
                    type:'Post',
                    dataType:'json',
                    success:function(data){
                        if(data.status == 0){
        ...
        buttons:{
          'Copy':function(){
                ...
                if(copy_status.length){
                    $.ajax({
                        url:'index.php?route=fbaccount/publish_nophoto/ajax_data&token=<?php echo $token;?>',
                        type:'Post',
                        data:{ajax:1,action:'copy',status:copy_status.join()},
                        dataType:'json',
                        beforeSend:function(){
                            $('#copy-dialog .do-result').html('<div class="alert alert-warning"><img src="<?php echo TPL_IMG?>loading.gif"> Please Waiting ... </div>');
...
$(document).delegate('#copy-dialog input[name="copy_status"]','click',function(){
    var copy_status = [];
    $('#copy-dialog input[name="copy_status"]').each(function(){
        if($(this).is(':checked')){
            copy_status.push($(this).val());
