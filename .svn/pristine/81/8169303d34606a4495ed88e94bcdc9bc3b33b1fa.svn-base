<?php
class ControllerFbmessageNophoto extends Controller {
	private $error = array();

	public function index() {
		$this->language->load('fbmessage/nophoto');

		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('fbmessage/nophoto');
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $post_id) {
				$this->model_fbmessage_nophoto->deleteContribute($post_id);
			}
			...
			$this->response->redirect($this->url->link('fbmessage/nophoto', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function validateDelete() {
		if (!$this->user->hasPermission('modify', 'fbmessage/nophoto/delete')) {
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('fbmessage/nophoto', 'token=' . $this->session->data['token'] , 'SSL'),
        );
        $data['delete'] = $this->url->link('fbmessage/nophoto/delete', 'token=' . $this->session->data['token'].$url , 'SSL');
		$limit = 20;
        ...
        $total = $this->model_fbmessage_nophoto->getTotalContributes($filter_data);

        $results = $this->model_fbmessage_nophoto->getContributes($filter_data);
        ...
        foreach ($results as $result) {
        	...
        	if($lock){
                ...
                'href' => $this->url->link('fbmessage/nophoto/detail', 'token=' . $this->session->data['token'] .'&contribute_id='.$result['contribute_id'], 'SSL')
        		);
        	}else{
        		$action[] = array(
        ...
        $data['sort_product'] = $this->url->link('fbmessage/nophoto', 'token=' . $this->session->data['token'] . '&sort=mc.product_id' . $url, 'SSL');
        $data['sort_contribute_sn'] = $this->url->link('fbmessage/nophoto', 'token=' . $this->session->data['token'] . '&sort=mc.contribute_sn' . $url, 'SSL');
        $data['sort_submited_date'] = $this->url->link('fbmessage/nophoto', 'token=' . $this->session->data['token'] . '&sort=mc.submited_date' . $url, 'SSL');
        $data['sort_date_modified'] = $this->url->link('fbmessage/nophoto', 'token=' . $this->session->data['token'] . '&sort=mc.date_modified' . $url, 'SSL');
        $data['sort_user'] = $this->url->link('fbmessage/nophoto', 'token=' . $this->session->data['token'] . '&sort=user' . $url, 'SSL');
        $data['sort_status'] = $this->url->link('fbmessage/nophoto', 'token=' . $this->session->data['token'] . '&sort=mc.status' . $url, 'SSL');
        $data['sort_publish'] = $this->url->link('fbmessage/nophoto', 'token=' . $this->session->data['token'] . '&sort=mc.publish' . $url, 'SSL');
        $data['sort_entry_sn'] = $this->url->link('fbmessage/nophoto', 'token=' . $this->session->data['token'] . '&sort=mc.entry_sn' . $url, 'SSL');
        $data['sort_author'] = $this->url->link('fbmessage/nophoto', 'token=' . $this->session->data['token'] . '&sort=mc.author_id' . $url, 'SSL');
        ...
        $pagination->url = $this->url->link('fbmessage/nophoto', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
        ...
        $this->response->setOutput($this->load->view('fbmessage/handling/nophoto_list.tpl', $data));
	}
	...
	public function detail(){
        ...
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->url->link('fbmessage/nophoto', 'token=' . $this->session->data['token'] . $url, 'SSL')
        );
        $data['cancel'] = $this->url->link('fbmessage/nophoto', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $data['action'] = $this->url->link('fbmessage/nophoto/save', '&token=' . $this->session->data['token'] , 'SSL');
        ...
        $_post = $this->model_fbmessage_nophoto->getContribute($this->request->get['contribute_id']); 
        ...
        $data['approve'] = in_array($this->user->getId(), $superior) && !in_array($_post['publish'], $this->config->get('fbmessage_promotion_modify'));
        ...
        if(empty($data['lock_user']) || $_post['locker'] == $this->user->getId()){
            $this->model_fbmessage_nophoto->setTempLocker((int)$_post['contribute_id']); 
        }else{
            $data['locked'] = true;
            $data['approve'] = false;
        ...
        $this->response->setOutput($this->load->view('fbmessage/handling/nophoto_form.tpl', $data));
	...
	public function history(){
		...
		$total = $this->model_fbmessage_nophoto->getTotalHistory($contribute_id);
		$results = $this->model_fbmessage_nophoto->getHistories($contribute_id,($page - 1) * $limit, $limit);
		foreach ($results as $result) {
		...
		$pagination->url = $this->url->link('fbmessage/nophoto/history', 'token=' . $this->session->data['token'] . '&contribute_id='.$contribute_id . '&page={page}', 'SSL');
		$data['pagination'] = $pagination->render();
		$data['results'] = sprintf($this->language->get('text_pagination'), ($total) ? (($page - 1) * $limit) + 1 : 0, ((($page - 1) * $limit) > ($total - $limit)) ? $total : ((($page - 1) * $limit) + $limit), $total, ceil($total / $limit));
		
		$this->response->setOutput($this->load->view('fbmessage/handling/nophoto_history.tpl', $data));
	}
	...
	public function save(){
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			$_post = $this->model_fbmessage_nophoto->getContribute($this->request->post['contribute_id']);
			...
			$this->model_fbmessage_nophoto->approve($tmp);
	...
	function ajax_data(){
		...
		switch ($action) {
			case 'reset':
				$this->model_fbmessage_nophoto->resetTempLocker($this->request->post['contribute_id'],$this->request->post['locker']);
				if(isset($this->request->get['set']) && $this->request->get['set']){
					$this->model_fbmessage_nophoto->setTempLocker($this->request->post['contribute_id']);
				}
