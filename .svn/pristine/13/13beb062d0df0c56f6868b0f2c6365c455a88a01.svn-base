...
class ControllerToolBackup extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->user->hasPermission('tool/backup')) {
		...
		$data['text_structure_data'] = $this->language->get('text_structure_data');
	...
	public function backup() {
		...
		if (!isset($this->request->post['backup'])) {
			...
			if(!empty($this->request->post['redirect'])){
				$route = trim($this->request->post['redirect']);
			}
			$this->response->redirect($this->url->link($route, 'token=' . $this->session->data['token'], 'SSL'));
		} elseif ($this->user->hasPermission('tool/backup/backup')) {
			...
			$this->response->setOutput($this->model_tool_backup->backup($this->request->post['backup'],$this->request->post['mode']));
		} else {
