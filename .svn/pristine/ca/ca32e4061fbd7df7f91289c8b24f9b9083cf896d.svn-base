<?php
class ModelFbmessageProfileNophoto extends Model {
	private $post_type = 2;
	...
	public function getContributes($data) {
		...
		$sql = "SELECT p.* FROM " . DB_PREFIX . "fbmessage_nophoto_post p WHERE p.author_id = '".$this->db->escape($this->user->getAuthorId())."' ";
		$implode = array();
	...
	public function getTotalContributes($data) {
		$sql = "SELECT COUNT(p.contribute_id) AS total  FROM " . DB_PREFIX . "fbmessage_nophoto_post p WHERE p.author_id = '".$this->db->escape($this->user->getAuthorId())."' ";
		$implode = array();
	...
	public function getContributeBySn($contribute_sn){
		$sql = "SELECT * FROM " . DB_PREFIX . "fbmessage_nophoto_post WHERE contribute_sn = '" . $this->db->escape($contribute_sn) . "' AND author_id = '".$this->db->escape($this->user->getAuthorId())."' ";
		$query = $this->db->query($sql);
		return $query->row;
	}
	public function getContribute($contribute_id) {
		$sql = "SELECT * FROM " . DB_PREFIX . "fbmessage_nophoto_post WHERE contribute_id = '" . (int)$contribute_id . "' AND author_id = '".$this->db->escape($this->user->getAuthorId())."' ";
		$query = $this->db->query($sql);
		return $query->row;
	}
	...
	public function getHistories($contribute_id,$start=0,$limit=20){
		$sql = "SELECT h.*,u.nickname FROM ".DB_PREFIX."fbmessage_nophoto_post_history h LEFT JOIN ".DB_PREFIX."user u ON h.user_id = u.user_id WHERE h.type = 'edit' AND h.contribute_id = '".(int)$contribute_id."' ORDER BY h.date_added DESC LIMIT " . (int)$start. "," . (int)$limit;
		$query = $this->db->query($sql);
	...
	public function getTotalHistory($contribute_id){
		$query = $this->db->query("SELECT COUNT(history_id) total FROM ".DB_PREFIX."fbmessage_nophoto_post_history WHERE type = 'edit' AND contribute_id = '".(int)$contribute_id."'");
		return $query->row['total'];
	}
