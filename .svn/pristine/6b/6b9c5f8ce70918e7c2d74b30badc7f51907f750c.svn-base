...
class ControllerUserUser extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm('user/user/add')) {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm('user/user/edit')) {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$roles_text = $workers =array();
			$authors = explode(",", $result['worker']);
			if(is_array($authors)){
				foreach ($authors as $user_id) {
					$_c = $this->model_user_user->getUser($user_id);
					if(isset($_c['nickname'])){
						$workers[] = $_c['nickname'];
					}
				}
			}
			$roles = $this->model_user_user->getUserRoles($result['user_id']);

			foreach($roles as $key){
				$roles_text[] = $this->language->get('text_group_'.strtolower($key));
			}
			$data['users'][] = array(
		...
		$data['column_workers'] = $this->language->get('column_workers');
		...
		$data['sort_author_id'] = $this->url->link('user/user', 'token=' . $this->session->data['token'] . '&sort=author_id' . $url, 'SSL');
		$data['sort_group'] = $this->url->link('user/user', 'token=' . $this->session->data['token'] . '&sort=group' . $url, 'SSL');
	...
	protected function getForm() {
		...
		$data['authors'] = $this->model_user_user->getAuthors();
