...
class PHPZip{
    ...
    // ------------------------------------------------------ //
    public function ZipAndDownload($dir,$zip_filename=''){
        if(@!function_exists('gzcompress')){ return; }
        @set_time_limit("0"); 
        ob_start();
		ob_get_contents();
        ...
        $out = $this->file();
		$filename = $zip_filename ? $zip_filename : 'Farticle'.date("YmdHis", time()).'.zip';
    ...
    private function ExtractFile($header, $to, $zip,$only_basename=false,$random_suffixes=false,$suffix_len=4){
        ...
        if($random_suffixes){
            $pathinfo = pathinfo($header['filename']);
            if(isset($pathinfo['filename']) && isset($pathinfo['extension'])){
                $header['filename'] = ($only_basename ? "" :rtrim($pathinfo['dirname'],"/").'/' ).$pathinfo['filename'].'_'.substr(md5(uniqid()),rand(1,16),$suffix_len).'.'.$pathinfo['extension'];
            }
        }
        ...
        return $random_suffixes ? $header['filename'] : true;
    }
    ...
    //}
    // ------------------------------------------------------ //
    public function unZip($zipfile, $to, $index = array(-1),$only_basename=false,$random_suffixes=false,$suffix_len=4){
        $ok  = 0;
        ...
        for($i=0; $i<$cdir['entries']; $i++){
            ...
            if(in_array("-1", $index) || in_array($i, $index)){
                $stat[$header['filename']] = $this->ExtractFile($header, $to, $zip,$only_basename,$random_suffixes,$suffix_len);
            }
        }
