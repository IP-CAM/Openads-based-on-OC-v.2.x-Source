...
class DB {
	...
	public function update($table,$condition,$data=array(),$dump= false){
		if(!empty($table)){
			$fields  = array();
			foreach ($data as $key => $value) {
				if(is_array($value)){
					...
					if(strtoupper($value['logic'])=='SELF'){
						$fields[] = (!empty($data['alias']) ? $data['alias']."." : '')."`".$key."` = ".$value['value'];
					}else{
						$fields[] = (!empty($data['alias']) ? $data['alias']."." : '')."`".$key."` = '".$this->escape(trim($value))."'";
					}
				}else{
					$fields[] = " `".$key."` = '".$this->escape(trim($value))."'";	
				}				
			...
			$sql = "UPDATE ".DB_PREFIX.trim($table)." SET ".implode(",",$fields).$wheresql;
